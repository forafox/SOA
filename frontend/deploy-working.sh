...
# Проверяем, что API routes созданы
if [ ! -d "out/api" ]; then
    echo "⚠️  Предупреждение: API routes не найдены в статической сборке"
    echo "   Это нормально для статического экспорта Next.js"
    echo "   API routes будут работать только в режиме сервера"
fi
