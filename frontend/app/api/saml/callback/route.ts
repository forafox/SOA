...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.log('Using origin:', envOrigin);
    
    if (!envOrigin || envOrigin === 'null' || envOrigin === 'undefined') {
      console.error('Invalid NEXT_PUBLIC_APP_ORIGIN:', process.env.NEXT_PUBLIC_APP_ORIGIN);
      return new NextResponse('Configuration error: Invalid APP_ORIGIN', { status: 500 });
    }
